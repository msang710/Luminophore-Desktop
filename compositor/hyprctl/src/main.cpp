...
std::string getRuntimeDir() {
    ...
    if (!XDG) {
        ...
        return "/run/user/" + USERID + "/luminophore";
    }

    return std::string{XDG} + "/luminophore";
}
...
int request(std::string_view arg, int minArgs = 0, bool needRoll = false) {
    ...
    if (instanceSignature.empty()) {
        log("LUMINOPHORE_INSTANCE_SIGNATURE was not set! (Is Hyprland running?) (3)");
        return 3;
    }
...
int requestIPC(std::string_view filename, std::string_view arg) {
    ...
    if (instanceSignature.empty()) {
        log("LUMINOPHORE_INSTANCE_SIGNATURE was not set! (Is Hyprland running?)");
        return 2;
    }
...
int main(int argc, char** argv) {
    ...
    if (overrideInstance.contains("_"))
        ...
        const auto ISIG = getenv("LUMINOPHORE_INSTANCE_SIGNATURE");

        if (!ISIG) {
            log("LUMINOPHORE_INSTANCE_SIGNATURE not set! (is hyprland running?)\n");
            return 1;
        }
    ...
    const auto SESSION_INSTANCE = getenv("LUMINOPHORE_INSTANCE_SIGNATURE");
    if (instanceSignature.empty() || instanceSignature == "." || instanceSignature == ".." || instanceSignature.find_first_of("/\\") != std::string::npos ||
        (SESSION_INSTANCE && instanceSignature != SESSION_INSTANCE)) {
        log("invalid instance or instance belongs to another session\n");
        return 1;
    }
