...
static std::expected<std::string, std::string> wallpaperSocketPath() {
    const auto RTDIR = getenv("XDG_RUNTIME_DIR");
    if (!RTDIR || RTDIR[0] == '\0')
        return std::unexpected("can't send: no XDG_RUNTIME_DIR");

    const auto INSTANCE = getenv("LUMINOPHORE_INSTANCE_SIGNATURE");
    if (!INSTANCE || INSTANCE[0] == '\0')
        return std::unexpected("can't send: no LUMINOPHORE_INSTANCE_SIGNATURE (not running under Luminophore)");

    const std::string_view signature{INSTANCE};
    if (signature == "." || signature == ".." || signature.find('/') != std::string_view::npos || signature.find('\\') != std::string_view::npos)
        return std::unexpected("can't send: invalid Luminophore instance signature");

    return RTDIR + "/luminophore/"s + INSTANCE + "/"s + SOCKET_NAME;
}
...
static std::expected<void, std::string> doWallpaper(const std::string_view& RHS) {
    ...
    const auto socketPath = wallpaperSocketPath();
    if (!socketPath)
        return std::unexpected(socketPath.error());
