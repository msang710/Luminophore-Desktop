...
TEST(Helpers, privateReleaseDoesNotReportHostPkgconfigAsRuntimeVersion) {
    const auto*       PREVIOUS = std::getenv("LUMINOPHORE_RELEASE_ROOT");
    const std::string SAVED    = PREVIOUS ? PREVIOUS : "";
    const bool        HAD_ROOT = PREVIOUS != nullptr;
    setenv("LUMINOPHORE_RELEASE_ROOT", "/nonexistent/private-release", 1);
    EXPECT_EQ(getSystemLibraryVersion("aquamarine"), "unknown (private release; host pkg-config excluded)");
    EXPECT_EQ(getSystemLibraryVersion("hyprutils"), "unknown (private release; host pkg-config excluded)");
    if (HAD_ROOT)
        setenv("LUMINOPHORE_RELEASE_ROOT", SAVED.c_str(), 1);
    else
        unsetenv("LUMINOPHORE_RELEASE_ROOT");
}
