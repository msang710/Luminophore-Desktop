...
TEST(MonitorPositionController, xwaylandPreservesReversedLayoutAndSpanningBounds) {
    const auto right          = testMonitor(0, "DP-1", {1920, 1080});
    const auto left           = testMonitor(1, "DP-2", {1920, 1080});
    right->m_explicitPosition = Vector2D{1920, 0};
    left->m_explicitPosition  = Vector2D{0, 0};
    arrange({right, left}, true);
    EXPECT_EQ(left->m_xwaylandPosition, Vector2D(0, 0));
    EXPECT_EQ(right->m_xwaylandPosition, Vector2D(1920, 0));
    const auto report = Vector2D{0, 0} - left->m_position + left->m_xwaylandPosition;
    EXPECT_EQ(report, Vector2D(0, 0));
    EXPECT_EQ(report.x + 3840, right->m_xwaylandPosition.x + 1920);
    arrange({left, right}, true);
    EXPECT_EQ(left->m_xwaylandPosition, Vector2D(0, 0));
    EXPECT_EQ(right->m_xwaylandPosition, Vector2D(1920, 0));
}

TEST(MonitorPositionController, xwaylandPreservesVerticalNegativeOriginAtCommonScale) {
    const auto bottom          = testMonitor(0, "bottom", {960, 540});
    const auto top             = testMonitor(1, "top", {960, 540});
    bottom->m_explicitPosition = Vector2D{-960, 0};
    top->m_explicitPosition    = Vector2D{-960, -540};
    for (auto m : {bottom, top}) {
        m->m_scale           = 2.F;
        m->m_transformedSize = Vector2D{1920, 1080};
    }
    arrange({bottom, top}, true);
    EXPECT_EQ(top->m_xwaylandPosition, Vector2D(0, 0));
    EXPECT_EQ(bottom->m_xwaylandPosition, Vector2D(0, 1080));
    EXPECT_FLOAT_EQ(top->m_xwaylandScale, 2.F);
    arrange({bottom, top}, false);
    EXPECT_EQ(top->m_xwaylandPosition, Vector2D(0, 0));
    EXPECT_EQ(bottom->m_xwaylandPosition, Vector2D(0, 540));
    EXPECT_FLOAT_EQ(top->m_xwaylandScale, 1.F);
}

TEST(MonitorPositionController, xwaylandLogicalLayoutSurvivesMixedScaleAndReconnect) {
    const auto right          = testMonitor(0, "right", {100, 100});
    const auto left           = testMonitor(1, "left", {100, 100});
    right->m_explicitPosition = Vector2D{0, 0};
    left->m_explicitPosition  = Vector2D{-100, 0};
    right->m_scale            = 2.F;
    arrange({right, left}, false);
    EXPECT_EQ(right->m_xwaylandPosition, Vector2D(100, 0));
    arrange({right}, false);
    EXPECT_EQ(right->m_xwaylandPosition, Vector2D(0, 0));
    arrange({left, right}, false);
    EXPECT_EQ(right->m_xwaylandPosition, Vector2D(100, 0));
    EXPECT_EQ(left->m_xwaylandPosition, Vector2D(0, 0));
}
