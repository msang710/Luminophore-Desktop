...
TEST(MonitorParser, OutputVrrChangeRequiresApplyingSoftProperties) {
    CMonitorRule before, after;
    EXPECT_EQ(before.compare(after), COMPARISON_FULL_MATCH);
    after.m_vrr = 2;
    EXPECT_EQ(before.compare(after), COMPARISON_SOFT_MISMATCH);
    before.m_vrr = 2;
    EXPECT_EQ(before.compare(after), COMPARISON_FULL_MATCH);
    after.m_vrr.reset();
    EXPECT_EQ(before.compare(after), COMPARISON_SOFT_MISMATCH);
