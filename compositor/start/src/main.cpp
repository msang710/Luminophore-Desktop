...
constexpr const char* HELP_INFO = R"#(Luminophore watchdog - Starts the explicitly selected compositor generation.
Any arguments after -- are passed to that compositor.

Additional arguments for start-hyprland:
 --path [path]       -> Required absolute path to the generation's compositor
...
int main(int argc, const char** argv, const char** envp) {
    ...
    // Resolve once, before probing or restarting. Never discover a different
    // compositor through PATH or a mutable current-generation symlink later.
    std::error_code ec;
    if (!g_state->customPath || !std::filesystem::path(*g_state->customPath).is_absolute()) {
        g_logger->log(LOG_ERR, "Luminophore watchdog requires --path with an absolute compositor path");
        return 1;
    }
    const auto executable = std::filesystem::canonical(*g_state->customPath, ec);
    if (ec || !std::filesystem::is_regular_file(executable, ec) || ec || access(executable.c_str(), X_OK) != 0) {
        g_logger->log(LOG_ERR, "Luminophore compositor is unavailable");
        return 1;
    }
    g_state->customPath = executable.string();
