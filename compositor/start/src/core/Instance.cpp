...
void CHyprlandInstance::runHyprlandThread(bool safeMode, bool lockedCrash) {
    ...
    std::vector<char*> args = {strdup(g_state->customPath.value().c_str())};
    ...
    if (forkRet == 0) {
        ...
        if (Nix::shouldUseNixGL()) {
            argsStd.insert(argsStd.begin(), g_state->customPath.value());
            args.insert(args.begin(), strdup(argsStd.front().c_str()));
            execvp("nixGL", args.data());
        } else
            execv(g_state->customPath.value().c_str(), args.data());
