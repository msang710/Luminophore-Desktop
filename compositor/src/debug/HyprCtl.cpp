#include "../config/luminophore/SettingsService.hpp"
#include "../config/luminophore/DesktopSettings.hpp"
#include "../config/luminophore/LuaSettingsRuntime.hpp"
#include "../config/luminophore/SettingsProtocol.hpp"
#include "../luminophore/LuminophoreMonitorLayout.hpp"
#include "../luminophore/LuminophoreLaunchOrigin.hpp"
#include "../luminophore/LuminophoreLivePipController.hpp"
...
std::string CHyprCtl::getMonitorData(PHLMONITOR m, eHyprCtlOutputFormat format) {
    ...
    "hardwareCursorsInUse": {}
            ...
            (!m->m_activeWorkspace ? "" : escapeJSONStrings(m->m_activeWorkspace->m_name)), (!m->m_activeWorkspace ? "legacy" : m->m_activeWorkspace->roleName()),
            m->activeSpecialWorkspaceID(), escapeJSONStrings(m->m_activeSpecialWorkspace ? m->m_activeSpecialWorkspace->m_name : ""),
            (!m->m_activeSpecialWorkspace ? "legacy" : m->m_activeSpecialWorkspace->roleName()), sc<int>(m->m_reservedArea.left()), sc<int>(m->m_reservedArea.top()),
...
std::string CHyprCtl::getWindowData(PHLWINDOW w, eHyprCtlOutputFormat format) {
    const auto PLACEMENT = w->isMinimized() ? "minimized" : (w->m_isFloating ? "floating" : "tiled");
    ...
    if (format == eHyprCtlOutputFormat::FORMAT_JSON) {
        const auto& RESTORE = w->m_restorePlacement;
        return std::format(
            R"#({{
    "address": "0x{:x}",
    "mapped": {},
    ...
    "stableId": "{:x}"
            ...
            escapeJSONStrings(!w->m_workspace ? "" : w->m_workspace->m_name), (!w->m_workspace ? "legacy" : w->m_workspace->roleName()),
            (sc<int>(w->m_isFloating) == 1 ? "true" : "false"), PLACEMENT, RESTORE && RESTORE->workspace ? RESTORE->workspace->m_id : WORKSPACE_INVALID,
            escapeJSONStrings(RESTORE && RESTORE->workspace ? RESTORE->workspace->m_name : ""), RESTORE && RESTORE->workspace ? RESTORE->workspace->roleName() : "legacy",
            escapeJSONStrings(RESTORE && RESTORE->monitor ? RESTORE->monitor->m_name : ""), RESTORE && RESTORE->wasFloating ? "true" : "false", w->monitorID(),
            escapeJSONStrings(w->m_class), escapeJSONStrings(w->m_title), escapeJSONStrings(w->m_initialClass), escapeJSONStrings(w->m_initialTitle), w->getPID(),
            (sc<int>(w->m_isX11) == 1 ? "true" : "false"), (w->m_pinned ? "true" : "false"), (w->m_pinFullscreened ? "true" : "false"),
            ...
            escapeJSONStrings(Fullscreen::controller()->getFullscreenHandlerNameAsString(w)), (w->m_allowedOverFullscreen ? "true" : "false"), getTagsData(w, format),
            ...
            (w->m_tearingHint ? "true" : "false"), w->m_stableID);
    } else {
        return std::format(
            "Window {:x} -> {}:\n\tmapped: {}\n\thidden: {}\n\tvisible: {}\n\tacceptsInput: {}\n\tat: {},{}\n\tsize: {},{}\n\tworkspace: {} ({})\n\tfloating: {}\n\tplacement: "
            "{}\n\tmonitor: "
            ...
            w->m_workspace ? w->workspaceID() : WORKSPACE_INVALID, (!w->m_workspace ? "" : w->m_workspace->m_name), sc<int>(w->m_isFloating), PLACEMENT, w->monitorID(), w->m_class,
            w->m_title, w->m_initialClass, w->m_initialTitle, w->getPID(), sc<int>(w->m_isX11), sc<int>(w->m_pinned), sc<int>(w->m_pinFullscreened),
            sc<uint8_t>(Fullscreen::controller()->getFullscreenModes(w).internal), sc<uint8_t>(Fullscreen::controller()->getFullscreenModes(w).client),
            Fullscreen::controller()->getFullscreenHandlerNameAsString(w), sc<int>(w->m_allowedOverFullscreen), getTagsData(w, format), rc<uintptr_t>(w->m_swallowee.get()),
            getFocusHistoryID(w), sc<int>(g_pInputManager->isWindowInhibiting(w, false)), w->xdgTag().value_or(""), w->xdgDescription().value_or(""),
            NContentType::toString(w->getContentType()), sc<int>(w->m_tearingHint), w->m_stableID);
    }
}
...
std::string CHyprCtl::getWorkspaceData(PHLWORKSPACE w, eHyprCtlOutputFormat format) {
    ...
    const std::string layoutName = "luminophore-spatial";
    ...
    "tiledLayout": "{}"
                           ...
                           w->m_id, escapeJSONStrings(w->m_name), w->roleName(), escapeJSONStrings(PMONITOR ? PMONITOR->m_name : "?"),
...
static std::string getWorkspaceRuleData(const Config::CWorkspaceRule& r, eHyprCtlOutputFormat format) {
    ...
    "workspaceString": "{}"{}{}{}{}{}{}{}{}{}{}{}
                        ...
                        escapeJSONStrings(r.m_workspaceString), enabled, monitor, persistent, gapsIn, gapsOut, borderSize, border, rounding, decorate, shadow, onCreatedEmpty);
        ...
        std::string result = std::format("Workspace rule {}:\n{}{}{}{}{}{}{}{}{}{}{}\n", escapeJSONStrings(r.m_workspaceString), enabled, monitor, persistent, gapsIn, gapsOut,
                                         borderSize, border, rounding, decorate, shadow, onCreatedEmpty);
...
static std::string dispatchGetProp(eHyprCtlOutputFormat format, std::string request) {
    ...
    auto borderColorToString = [&](bool active) -> std::string {
        ...
        if (active) {
            ...
            std::string borderColorString = PWINDOW->m_ruleApplicator->activeBorderColor().valueOr(*ACTIVECOL).toString();
            if (FORMNORM)
            ...
            std::string borderColorString = PWINDOW->m_ruleApplicator->inactiveBorderColor().valueOr(*INACTIVECOL).toString();
            if (FORMNORM)
                return borderColorString;
            else
                return std::format(R"({{"{}": "{}"}})", PROP, borderColorString);
...
#ifdef LUMINOPHORE_EFFECTS
static std::string luminophoreFrameReason(Render::eLuminophoreFrameReason reason) {
    switch (reason) {
        case Render::LUMINOPHORE_FRAME_WINDOW_GLOW_ACTIVE: return "window-glow-active";
        case Render::LUMINOPHORE_FRAME_WINDOW_MODE_TRANSITION: return "window-mode-transition";
        case Render::LUMINOPHORE_FRAME_SHELL_OVERLAY_VISIBLE: return "shell-overlay-visible";
        case Render::LUMINOPHORE_FRAME_SHELL_TRANSITION: return "shell-transition";
    }
    return "unknown";
}

static std::string luminophoreDiagnosticsRequest(eHyprCtlOutputFormat format, std::string request) {
    if (!g_pHyprRenderer || !g_pHyprRenderer->m_luminophoreWindowEffect)
        return format == FORMAT_JSON ? "{\"enabled\":false,\"events\":[]}" : "LUMINOPHORE effects disabled\n";

    const auto  counters = g_pHyprRenderer->m_luminophoreWindowEffect->diagnostics();
    const auto  events   = g_pHyprRenderer->m_luminophoreWindowEffect->frameEvents();
    const auto  shell    = Luminophore::shellProjection()->bloomDiagnostics();
    std::string result;

    if (format == FORMAT_JSON) {
        result = std::format("{{\"enabled\":true,\"enqueued\":{},\"drawn\":{},\"frameRequests\":{},\"shaderFailures\":{},\"skipped\":{},"
                             "\"shellBloom\":{{\"caches\":{},\"rendered\":{},\"regenerations\":{},\"allocations\":{},\"fallbacks\":{}}},\"events\":[",
                             counters.enqueued, counters.drawn, counters.frameRequests, counters.shaderFailures, counters.skipped, shell.caches, shell.rendered,
                             shell.regenerations, shell.allocations, shell.fallbacks);
        for (const auto& event : events)
            result +=
                std::format("{{\"monitor\":{},\"sequence\":{},\"reason\":\"{}\",\"requestedAtUs\":{},\"presentedAtUs\":{},\"damageArea\":{},\"coalesced\":{}}},", event.monitor,
                            event.sequence, luminophoreFrameReason(event.reason), event.requestedAtUs, event.presentedAtUs, event.damageArea, event.coalesced ? "true" : "false");
        trimTrailingComma(result);
        return result + "]}";
    }

    result = std::format(
        "enqueued={} drawn={} frameRequests={} shaderFailures={} skipped={} shellCaches={} shellRendered={} shellRegenerations={} shellAllocations={} shellFallbacks={}\n",
        counters.enqueued, counters.drawn, counters.frameRequests, counters.shaderFailures, counters.skipped, shell.caches, shell.rendered, shell.regenerations, shell.allocations,
        shell.fallbacks);
    for (const auto& event : events)
        result += std::format("monitor={} sequence={} reason={} requested_us={} presented_us={} damage_area={} coalesced={}\n", event.monitor, event.sequence,
                              luminophoreFrameReason(event.reason), event.requestedAtUs, event.presentedAtUs, event.damageArea, event.coalesced);
    return result;
}
#endif

static std::string luminophoreShellProjectionStateRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    const auto name      = separator == std::string::npos ? std::string{} : request.substr(separator + 1);
    if (!name.starts_with("luminophore-shell-") || name.size() > 128 || name.find_first_of(" \t\r\n") != std::string::npos)
        return R"({"error":"invalid namespace"})";
    std::string result = R"({"version":1,"receipts":[)";
    for (const auto& receipt : Luminophore::shellProjection()->receiptsFor(name)) {
        if (result.back() != '[')
            result += ',';
        const auto state = receipt.state == Luminophore::SHELL_SNAPSHOT_PRESENTED ? "presented" : receipt.state == Luminophore::SHELL_SNAPSHOT_COMMITTED ? "committed" : "prepared";
        result += std::format(R"({{"generation":"{}","revision":{},"contentRevision":{},"state":"{}"}})", escapeJSONStrings(receipt.generation), receipt.revision,
                              receipt.contentRevision, state);
    }
    return result + "]}";
}

static std::string luminophoreSettingsServiceRequest(eHyprCtlOutputFormat, std::string request) {
    auto& service = Luminophore::Settings::settingsService();
    if (!service)
        return R"({"error":"settings service unavailable"})";
    const auto separator = request.find(' ');
    auto       reply     = service->request(separator == std::string::npos ? "status" : request.substr(separator + 1));
    if (reply.ends_with('}')) {
        reply.pop_back();
        reply += std::format(",\"requiresSessionRestart\":{}}}", Luminophore::Settings::desktopSessionRestartRequired() ? "true" : "false");
    }
    return reply;
}

static std::string luminophoreSettingsValidateRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    return Luminophore::Settings::validateRequest(separator == std::string::npos ? std::string_view{} : std::string_view{request}.substr(separator + 1));
}

static std::string luminophoreSpatialPreviewRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    const auto command   = separator == std::string::npos ? std::nullopt : CLuminophoreSpatialEditorProtocol::parsePreview(request.substr(separator + 1));
    if (!command)
        return CLuminophoreSpatialEditorProtocol::serialize({.status = eLuminophoreSpatialTransactionStatus::INVALID_COMMAND});
    return CLuminophoreSpatialEditorProtocol::serialize(Luminophore::spatialRuntime()->edit(*command, true));
}

static std::string luminophoreSpatialCommitRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    const auto command   = separator == std::string::npos ? std::nullopt : CLuminophoreSpatialEditorProtocol::parsePreview(request.substr(separator + 1));
    if (!command)
        return R"({"error":"invalid spatial edit request"})";
    return CLuminophoreSpatialEditorProtocol::serialize(Luminophore::spatialRuntime()->edit(*command, false));
}

static std::string luminophorePipCommandRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    const auto command   = separator == std::string::npos ? std::nullopt : Luminophore::CLuminophoreLivePipProtocol::parse(request.substr(separator + 1));
    if (!command)
        return R"({"error":"invalid PiP request"})";
    return Luminophore::livePipProtocol()->execute(*command);
}

static std::string luminophoreVisualSettingsRequest(eHyprCtlOutputFormat, std::string request) {
    const auto separator = request.find(' ');
    const auto command   = separator == std::string::npos ? std::nullopt : Luminophore::parseVisualCommand(std::string_view(request).substr(separator + 1));
    if (!command)
        return R"({"error":"invalid visual settings request"})";
    try {
        const auto revision = Luminophore::visualSettings()->revision();
        Luminophore::visualSettings()->apply(command->settings, command->receipt);
        if (revision != Luminophore::visualSettings()->revision())
            Luminophore::shellProjection()->visualSettingsChanged();
        return Luminophore::visualSettings()->json();
    } catch (const std::exception&) { return R"({"error":"visual settings request rejected"})"; }
}

static std::string luminophoreSpatialStateRequest(eHyprCtlOutputFormat format, std::string request) {
    const auto  state = Luminophore::spatialRuntime()->snapshot();
    std::string result;
    const auto  mode       = state.presentationMode == eLuminophorePresentationMode::DESKTOP ? "desktop" :
        state.presentationMode == eLuminophorePresentationMode::WIDE                         ? "wide" :
                                                                                               "normal";
    const auto  wideKey    = state.wideKey ? std::format("\"0x{:x}\"", *state.wideKey) : "null";
    const auto  focusedKey = state.focusedKey ? std::format("\"0x{:x}\"", *state.focusedKey) : "null";

    if (format == FORMAT_JSON) {
        result =
            std::format("{{\"protocolVersion\":2,\"active\":{},\"committed\":{},\"revision\":{},\"topologyRevision\":{},\"committedRevision\":{{\"model\":{},\"topology\":{}}},"
                        "\"presentationMode\":\"{}\",\"wideKey\":{},\"focusedKey\":{},\"selectedOutput\":{},"
                        "\"extent\":{{\"columns\":{},\"rows\":{}}},"
                        "\"displayOrigin\":{{\"x\":{},\"y\":{}}},"
                        "\"view\":{{\"x\":{},\"y\":{},\"columns\":{},\"rows\":{}}},\"outputViews\":[",
                        state.active ? "true" : "false", state.committed ? "true" : "false", state.revision, state.topologyRevision, state.committedModelRevision,
                        state.committedTopologyRevision, mode, wideKey, focusedKey, state.selectedOutputID, state.extent.columns, state.extent.rows, 0, 0, state.view.origin.x,
                        state.view.origin.y, state.view.columns, state.view.rows);
        for (const auto& outputView : state.outputViews) {
            const auto anchor = outputView.anchorKey ? std::format("\"0x{:x}\"", *outputView.anchorKey) : "null";
            result += std::format("{{\"output\":{},\"x\":{},\"y\":{},\"columns\":{},\"rows\":{},\"anchorKey\":{},\"board\":{}}},", outputView.outputID, outputView.rect.origin.x,
                                  outputView.rect.origin.y, outputView.rect.columns, outputView.rect.rows, anchor, outputView.boardID);
        }
        trimTrailingComma(result);
        result += std::format("],\"focusRevision\":{},\"meshes\":[", state.independent ? state.independent->focusRevision : 0);
        if (state.independent)
            for (const auto& [id, mesh] : state.independent->meshes)
                result += std::format("{{\"board\":{},\"revision\":{},\"lastResetRevision\":{}}},", id, mesh.revision,
                                      state.independent->meshResetRevisions.contains(id) ? state.independent->meshResetRevisions.at(id) : 0);
        trimTrailingComma(result);
        result += "],\"outputs\":[";
        for (const auto& output : state.outputs)
            result += std::format("{{\"id\":{},\"name\":\"{}\",\"box\":{{\"x\":{},\"y\":{},\"width\":{},\"height\":{}}},\"scaleMilli\":{},\"transform\":{}}},", output.id,
                                  escapeJSONStrings(output.name), output.box.x, output.box.y, output.box.width, output.box.height, output.scaleMilli, output.transform);
        trimTrailingComma(result);
        result += std::format("],\"capabilities\":[\"surface-source-v1\"],\"sourceInstance\":\"{}\",\"windows\":[", escapeJSONStrings(g_pCompositor->m_instanceSignature));
        for (const auto& entry : state.windows) {
            result += std::format("{{\"address\":\"0x{:x}\",\"mode\":\"{}\",\"x\":{},\"y\":{},\"visible\":{},"
                                  "\"primaryOutput\":{},\"board\":{},\"box\":{{\"x\":{},\"y\":{},\"width\":{},\"height\":{}}},\"fragments\":[",
                                  entry.key, entry.floating ? "floating" : "tiled", entry.coordinate.x, entry.coordinate.y, entry.visible ? "true" : "false", entry.primaryOutputID,
                                  entry.boardID, entry.committedBox.x, entry.committedBox.y, entry.committedBox.width, entry.committedBox.height);
            for (const auto& fragment : entry.fragments)
                result += std::format("{{\"output\":{},\"x\":{},\"y\":{},\"box\":{{\"x\":{},\"y\":{},\"width\":{},\"height\":{}}}}},", fragment.outputID, fragment.point.x,
                                      fragment.point.y, fragment.box.x, fragment.box.y, fragment.box.width, fragment.box.height);
            trimTrailingComma(result);
            const auto& source = entry.source;
            const auto  extent = source.extent ? std::format("{{\"width\":{},\"height\":{}}}", source.extent->width, source.extent->height) : "null";
            result += std::format("],\"presentationAvailable\":{},\"source\":{{\"token\":\"{}\",\"revision\":\"{}\",\"extentRevision\":\"{}\",\"alive\":{},\"mapped\":{},"
                                  "\"hasBuffer\":{},\"extent\":{}}}}},",
                                  entry.presentationAvailable, source.token, source.revision, source.extentRevision, source.alive, source.mapped, source.hasBuffer, extent);
        }
        trimTrailingComma(result);
        return result + "]}";
    }
    ...
    result = std::format(
        "active={} committed={} revision={} topologyRevision={} committedRevision={}/{} mode={} wideKey={} focusedKey={} selectedOutput={} extent={}x{} view={},{} {}x{}\n",
        state.active, state.committed, state.revision, state.topologyRevision, state.committedModelRevision, state.committedTopologyRevision, mode, wideKey, focusedKey,
        state.selectedOutputID, state.extent.columns, state.extent.rows, state.extent.columns / 2, state.extent.rows / 2, state.view.origin.x, state.view.origin.y,
        state.view.columns, state.view.rows);
    for (const auto& outputView : state.outputViews)
        result += std::format("outputView={} rect={},{} {}x{} anchor={}\n", outputView.outputID, outputView.rect.origin.x, outputView.rect.origin.y, outputView.rect.columns,
                              outputView.rect.rows, outputView.anchorKey ? std::format("0x{:x}", *outputView.anchorKey) : "none");
    for (const auto& output : state.outputs)
        result += std::format("output={} name={} box={},{},{}x{} scaleMilli={} transform={}\n", output.id, output.name, output.box.x, output.box.y, output.box.width,
                              output.box.height, output.scaleMilli, output.transform);
    for (const auto& entry : state.windows)
        result += std::format("address=0x{:x} mode={} coordinate={},{} visible={} primaryOutput={} fragments={} box={},{},{}x{}\n", entry.key,
                              entry.floating ? "floating" : "tiled", entry.coordinate.x, entry.coordinate.y, entry.visible, entry.primaryOutputID, entry.fragments.size(),
                              entry.committedBox.x, entry.committedBox.y, entry.committedBox.width, entry.committedBox.height);
    return result;
}
...
CHyprCtl::CHyprCtl() {
    ...
    registerCommand(SHyprCtlCommand{"luminophoremonitorlayout", false, [](eHyprCtlOutputFormat, std::string request) {
                                        const auto pos = request.find(' ');
                                        return Luminophore::MonitorLayout::request(pos == std::string::npos ? "snapshot" : request.substr(pos + 1));
                                    }});
    registerCommand(SHyprCtlCommand{"luminophorelaunchtoken", false, [](eHyprCtlOutputFormat, std::string request) {
                                        const auto separator = request.find(' ');
                                        return Luminophore::LaunchOrigin::issue(separator == std::string::npos ? "" : request.substr(separator + 1));
                                    }});
...
#ifdef LUMINOPHORE_EFFECTS
    registerCommand(SHyprCtlCommand{"luminophorediagnostics", true, luminophoreDiagnosticsRequest});
    registerCommand(SHyprCtlCommand{"luminophoresettingsservice", false, luminophoreSettingsServiceRequest});
    registerCommand(SHyprCtlCommand{"luminophoresettingsvalidate", false, luminophoreSettingsValidateRequest});
#endif
    registerCommand(SHyprCtlCommand{"luminophoreshellprojectionstate", false, luminophoreShellProjectionStateRequest});
    registerCommand(SHyprCtlCommand{"luminophorespatialpreview", false, luminophoreSpatialPreviewRequest});
    registerCommand(SHyprCtlCommand{"luminophorespatialcommit", false, luminophoreSpatialCommitRequest});
    registerCommand(SHyprCtlCommand{"luminophorecommand", false, [](eHyprCtlOutputFormat, std::string request) {
                                        const auto separator = request.find(' ');
                                        return separator == std::string::npos ? std::string{"error: missing command payload"} :
                                                                                Luminophore::Settings::desktopCommand(request.substr(separator + 1));
                                    }});
    registerCommand(SHyprCtlCommand{"luminophorepipcommand", false, luminophorePipCommandRequest});
    registerCommand(SHyprCtlCommand{"luminophorevisualsettings", false, luminophoreVisualSettingsRequest});
    registerCommand(SHyprCtlCommand{"luminophorevisualstate", true, [](eHyprCtlOutputFormat, std::string) { return Luminophore::visualSettings()->json(); }});
    registerCommand(SHyprCtlCommand{"luminophorespatialstate", true, [](eHyprCtlOutputFormat, std::string) {
                                        return std::string{"{\"active\":false,\"error\":\"Spatial protocol 2 requires luminophorespatialstate2 and the matching Shell\"}"};
                                    }});
    registerCommand(SHyprCtlCommand{"luminophorespatialhistory", true, [](eHyprCtlOutputFormat, std::string) {
                                        return std::format(R"({{"undo":{},"redo":{},"revision":{},"source":"{}"}})", Luminophore::spatialRuntime()->undoCount(),
                                                           Luminophore::spatialRuntime()->redoCount(), Luminophore::spatialRuntime()->revision(),
                                                           escapeJSONStrings(g_pCompositor->m_instanceSignature));
                                    }});
    registerCommand(SHyprCtlCommand{"luminophorespatialstate2", true, luminophoreSpatialStateRequest});
    registerCommand(SHyprCtlCommand{"luminophorelivepip", true, [](eHyprCtlOutputFormat, std::string) { return Luminophore::livePipController()->query(); }});
