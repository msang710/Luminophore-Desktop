...
CPrimarySelectionOffer::CPrimarySelectionOffer(SP<CZwpPrimarySelectionOfferV1> resource_, SP<IDataSource> source_) : m_source(source_), m_resource(resource_) {
    ...
    m_resource->setReceive([this](CZwpPrimarySelectionOfferV1* r, const char* mime, int32_t fd) {
        ...
        static auto     enabled = CConfigValue<Config::INTEGER>("misc:middle_click_paste");
        if (!*enabled)
            return;
        if (!m_source) {
...
void CPrimarySelectionProtocol::settingsChanged() {
    // Revoke offers without destroying the user's source; rollback can re-offer it.
    onPointerFocus();
}
