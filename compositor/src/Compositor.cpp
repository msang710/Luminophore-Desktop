...
CCompositor::CCompositor(bool onlyConfig) : m_onlyConfigVerification(onlyConfig), m_hyprlandPID(getpid()) {
    ...
    m_hyprTempDataRoot = std::string{getenv("XDG_RUNTIME_DIR")} + "/luminophore";

    if (m_hyprTempDataRoot.starts_with("/luminophore")) {
    ...
    setenv("LUMINOPHORE_INSTANCE_SIGNATURE", m_instanceSignature.c_str(), true);
...
//
static SP<CEventLoopTimer> g_settingsTimer;
...
void                       CCompositor::initServer(std::string socketName, int socketFd) {
    if (m_onlyConfigVerification) {
        ...
        if (!Config::initConfigManager())
            throw std::runtime_error("native settings verification failed");
        Config::mgr()->init();
    ...
    const bool headlessOnly = Env::envEnabled("HYPRLAND_HEADLESS_ONLY");
    const bool nestedOnly   = Env::envEnabled("LUMINOPHORE_NESTED_ONLY");
    if (!headlessOnly && !nestedOnly) {
    ...
    if (!headlessOnly) {
        ...
        option.backendRequestMode = nestedOnly ? Aquamarine::eBackendRequestMode::AQ_BACKEND_REQUEST_MANDATORY : Aquamarine::eBackendRequestMode::AQ_BACKEND_REQUEST_FALLBACK;
        implementations.emplace_back(option);
    }
    ...
    if (!getenv("XDG_CURRENT_DESKTOP")) {
        setenv("XDG_CURRENT_DESKTOP", "Luminophore", 1);
    ...
    Luminophore::Settings::initSettingsService();
    if (Luminophore::Settings::settingsService()) {
        g_settingsTimer = makeShared<CEventLoopTimer>(
            std::chrono::milliseconds(50),
            [](SP<CEventLoopTimer> self, void*) {
                self->updateTimeout(std::chrono::milliseconds(50));
                if (Luminophore::Settings::settingsService())
                    Luminophore::Settings::settingsService()->tick();
            },
            nullptr);
        g_pEventLoopManager->addTimer(g_settingsTimer);
    }
...
void CCompositor::cleanEnvironment() {
    ...
    // in startCompositor
    unsetenv("LUMINOPHORE_INSTANCE_SIGNATURE");
    ...
    if (m_aqBackend->hasSession() && !Env::envEnabled("HYPRLAND_NO_SD_VARS")) {
        const auto CMD =
#ifdef USES_SYSTEMD
            "systemctl --user unset-environment DISPLAY WAYLAND_DISPLAY LUMINOPHORE_INSTANCE_SIGNATURE XDG_CURRENT_DESKTOP QT_QPA_PLATFORMTHEME PATH XDG_DATA_DIRS && hash "
            "dbus-update-activation-environment 2>/dev/null && "
#endif
            "dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP LUMINOPHORE_INSTANCE_SIGNATURE QT_QPA_PLATFORMTHEME PATH XDG_DATA_DIRS";
...
void CCompositor::cleanup() {
    if (g_settingsTimer && g_pEventLoopManager)
        g_pEventLoopManager->removeTimer(g_settingsTimer);
    g_settingsTimer.reset();
    Luminophore::Settings::settingsService().reset();
    if (!m_wlDisplay)
        return;
...
void CCompositor::startCompositor() {
    ...
    if (
        ...
        const auto CMD =
#ifdef USES_SYSTEMD
            "systemctl --user import-environment DISPLAY WAYLAND_DISPLAY LUMINOPHORE_INSTANCE_SIGNATURE XDG_CURRENT_DESKTOP QT_QPA_PLATFORMTHEME PATH XDG_DATA_DIRS && hash "
            "dbus-update-activation-environment 2>/dev/null && "
#endif
            "dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP LUMINOPHORE_INSTANCE_SIGNATURE QT_QPA_PLATFORMTHEME PATH XDG_DATA_DIRS";
        Config::Supplementary::executor()->spawn(CMD);
...
void CCompositor::performUserChecks() {
    ...
    if (!*PNOCHECKXDG) {
        ...
        if (!CURRENT_DESKTOP_ENV || std::string{CURRENT_DESKTOP_ENV} != "Luminophore") {
