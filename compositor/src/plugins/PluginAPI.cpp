...
// Retained ABI symbols reject executable configuration extensions explicitly.
APICALL bool HyprlandAPI::addLuaFunction(HANDLE, const std::string&, const std::string&, PLUGIN_LUA_FN) {
    return false;
}
APICALL bool HyprlandAPI::removeLuaFunction(HANDLE, const std::string&, const std::string&) {
    return false;
}
