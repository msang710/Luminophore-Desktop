...
eDirectionalMoveResult CDefaultFloatingAlgorithm::moveTargetInDirection(SP<ITarget> t, Math::eDirection dir, bool silent) {
    if (!t)
    ...
    switch (dir) {
        ...
        default: Log::logger->log(Log::ERR, "Invalid direction in CDefaultFloatingAlgorithm::moveTargetInDirection"); return DIRECTIONAL_MOVE_INVALID_TARGET;
    }

    if (pos == originalPos)
        return DIRECTIONAL_MOVE_BLOCKED_EDGE;

    t->setPositionGlobal(pos);

    updateTarget(t);
    return DIRECTIONAL_MOVE_MOVED;
}
