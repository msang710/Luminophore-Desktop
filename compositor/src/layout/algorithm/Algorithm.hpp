...
namespace Layout {
    ...
    class CAlgorithm {
      public:
        ...
        void                          trackSpatialTiledTarget(SP<ITarget> target);
        void                          untrackSpatialTiledTarget(SP<ITarget> target);
        void                          setFloatingForSpatialTarget(SP<ITarget> target, bool floating);

        void                          swapTargets(SP<ITarget> a, SP<ITarget> b);
        eDirectionalMoveResult        moveTargetInDirection(SP<ITarget> t, Math::eDirection dir, bool silent);
        ...
        void                          recalculateFloating(eRecalculateReason reason = RECALCULATE_REASON_UNKNOWN);
