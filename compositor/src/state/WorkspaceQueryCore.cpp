...
std::optional<size_t> CWorkspaceQueryCore::run() && {
    if (m_string) {
        if (m_string->starts_with("name:luminophore-base-") && m_string->size() > std::string_view{"name:luminophore-base-"}.size())
            std::move(*this).name(m_string->substr(5));
        else if (*m_string == "special:luminophore-spotify")
