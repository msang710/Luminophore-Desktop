...
PHLWINDOW CViewHitTester::windowAt(const Vector2D& pos, uint16_t properties, PHLWINDOW ignoreWindow) const {
    ...
    const auto  spatialHitBox = [&](PHLWINDOW window, CBox box) -> std::optional<CBox> {
        if (!Luminophore::spatialRuntime()->isBoardRoot(window))
            return box;
        const auto regions = Luminophore::spatialRuntime()->regionsFor(window, PMONITOR);
        CRegion    region;
        for (const auto& r : regions)
            region.add(r);
        if (region.empty())
            return std::nullopt;
        return box.intersection(region.getExtents());
    };
    const auto spatialAcceptsPoint = [&](PHLWINDOW window, const Vector2D& point) {
        if (!Luminophore::spatialRuntime()->isBoardRoot(window))
            return true;
        const auto regions = Luminophore::spatialRuntime()->regionsFor(window, PMONITOR);
        return std::ranges::any_of(regions, [&](const auto& box) { return box.containsPoint(point); });
    };
    ...
    auto windowForWorkspace = [&](bool special) -> PHLWINDOW {
        auto floating = [&](bool aboveFullscreen) -> PHLWINDOW {
            for (auto const& w : WINDOWS | std::views::reverse) {
                ...
                if (PWINDOWMONITOR && PWINDOWMONITOR->m_activeSpecialWorkspace && w->m_workspace != PWINDOWMONITOR->m_activeSpecialWorkspace) {
                    const auto BB = w->getWindowBoxUnified(properties);
                ...
                const bool spatialRoot    = Luminophore::spatialRuntime()->isBoardRoot(w);
                const bool spatialVisible = spatialRoot && !Luminophore::spatialRuntime()->regionsFor(w, PMONITOR).empty();
                if (w->m_isFloating && w->m_isMapped && (spatialVisible || (!spatialRoot && w->m_workspace->isVisible())) && w->acceptsInput() && !w->m_pinned &&
                    ...
                    const auto BB          = w->getWindowBoxUnified(properties);
                    CBox       box         = BB.copy().expand(!w->isX11OverrideRedirect() ? BORDER_GRAB_AREA : 0);
                    const auto constrained = spatialHitBox(w, box);
                    if (!constrained)
                        continue;
                    box = *constrained;
                    if (HITBOX_SHRINK > 0 && w != LASTFOCUSED)
        ...
        if (Fullscreen::controller()->hasFullscreen(PWORKSPACE) && !(properties & SKIP_FULLSCREEN_PRIORITY) && !ONLY_PRIORITY &&
            !Luminophore::spatialRuntime()->isBoardRoot(Fullscreen::controller()->getFullscreenWindow(PWORKSPACE))) {
        ...
        for (auto const& w : WINDOWS) {
            ...
            const bool spatialRoot    = Luminophore::spatialRuntime()->isBoardRoot(w);
            const bool spatialVisible = spatialRoot && !Luminophore::spatialRuntime()->regionsFor(w, PMONITOR).empty();
            if (!w->m_isX11 && !w->m_isFloating && w->m_isMapped && (spatialVisible || (!spatialRoot && w->workspaceID() == WSPID)) && w->acceptsInput() &&
        ...
        for (auto const& w : WINDOWS) {
            ...
            const bool spatialRoot    = Luminophore::spatialRuntime()->isBoardRoot(w);
            const bool spatialVisible = spatialRoot && !Luminophore::spatialRuntime()->regionsFor(w, PMONITOR).empty();
            if (!w->m_isFloating && w->m_isMapped && (spatialVisible || (!spatialRoot && w->workspaceID() == WSPID)) && w->acceptsInput() && !w->m_X11ShouldntFocus &&
...
SP<CWLSurfaceResource> CViewHitTester::layerPopupSurfaceAt(const Vector2D& pos, PHLMONITOR monitor, Vector2D* surfaceCoords, PHLLS* layerFound) const {
    for (auto const& lsl : monitor->m_layerSurfaceLayers | std::views::reverse) {
        for (auto const& weak : lsl | std::views::reverse) {
            const auto ls = weak.lock();
            if (!ls || !ls->aliveAndVisible() || !ls->m_popupHead)
                continue;
...
SP<CWLSurfaceResource> CViewHitTester::layerPopupSurfaceAt(const Vector2D& pos, const std::vector<PHLLSREF>* layerSurfaces, Vector2D* surfaceCoords, PHLLS* layerFound) const {
    for (auto const& weak : *layerSurfaces | std::views::reverse) {
        const auto ls = weak.lock();
        if (!ls || !ls->aliveAndVisible() || !ls->m_popupHead)
            continue;
...
SP<CWLSurfaceResource> CViewHitTester::layerSurfaceAt(const Vector2D& pos, const std::vector<PHLLSREF>* layerSurfaces, Vector2D* surfaceCoords, PHLLS* layerFound,
                                                      bool aboveLockscreen) const {
    for (auto const& weak : *layerSurfaces | std::views::reverse) {
        // Client teardown may re-enter hit testing before weak list entries are erased.
        const auto ls = weak.lock();
        if (!ls || !ls->aliveAndVisible() || (aboveLockscreen && ls->m_ruleApplicator->aboveLock().valueOrDefault() != 2))
            continue;

        const auto layer = ls->m_layerSurface.lock();
        const auto root  = layer ? layer->m_surface.lock() : nullptr;
        if (!root)
            continue;
        auto [surf, local] = root->at(pos - ls->m_geometry.pos(), true);
