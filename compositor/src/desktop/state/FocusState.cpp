...
void CFocusState::fullWindowFocus(PHLWINDOW pWindow, eFocusReason reason, SP<CWLSurfaceResource> surface, bool forceFSCycle) {
    if (pWindow) {
        ...
        const auto FSWINDOW = Fullscreen::controller()->getFullscreenWindow(pWindow->m_workspace);
        if (FSWINDOW && !Luminophore::spatialRuntime()->isBoardRoot(pWindow) && !Fullscreen::controller()->layoutManagedFS(FSWINDOW)) {
...
void CFocusState::rawWindowFocus(PHLWINDOW pWindow, eFocusReason reason, SP<CWLSurfaceResource> surface) {
    if (reason == FOCUS_REASON_NEW_WINDOW && pWindow && Luminophore::spatialRuntime()->isBoardRoot(pWindow))
        return;
    if (pWindow && Luminophore::spatialRuntime()->desktopExposed())
        return;
    ...
    if (const auto drag = g_layoutManager->dragController()->target(); drag && drag->window() != pWindow)
    ...
    if (PMONITOR && PMONITOR->m_activeSpecialWorkspace && PMONITOR->m_activeSpecialWorkspace != pWindow->m_workspace && !pWindow->m_pinned)
    ...
    Luminophore::spatialRuntime()->focusChanged();
