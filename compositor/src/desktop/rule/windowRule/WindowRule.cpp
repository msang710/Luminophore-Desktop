...
static std::expected<WindowRuleEffectValue, std::string> parseWindowRuleEffect(CWindowRuleEffectContainer::storageType e, const std::string& raw) {
    ...
    switch (e) {
        ...
        case WINDOW_RULE_EFFECT_INITIAL_PLACEMENT:
            if (raw == "right" || raw == "left" || raw == "up" || raw == "down" || raw == "default")
                return raw;
            return std::unexpected("initial_placement must be default, right, left, up or down");
