...
bool CWindow::visible() const {
    return !isDesktopSuppressed() && m_isMapped && m_wlSurface && m_wlSurface->resource() && visibleByAlpha();
}
...
void CWindow::updateWindowDecos() {
    ...
    if (!m_isMapped || isDesktopSuppressed())
        return;
...
void CWindow::setHidden(bool hidden) {
    ...
    if (isDesktopSuppressed() && Desktop::focusState()->window() == m_self)
        Desktop::focusState()->window().reset();

    setSuspended(isDesktopSuppressed());
}
...
void CWindow::setSpatiallySuppressed(bool suppressed) {
    if (m_spatiallySuppressed == suppressed)
        return;

    m_spatiallySuppressed = suppressed;
    setSuspended(isDesktopSuppressed());
}

bool CWindow::isSpatiallySuppressed() const {
    return m_spatiallySuppressed;
}
...
bool CWindow::isDesktopSuppressed() const {
    return m_hidden || m_minimized || m_spatiallySuppressed || Luminophore::spatialRuntime()->desktopExposed();
}

bool Desktop::View::inputBlockRevokesFocus(uint32_t remainingReasons, bool desktopExposed) {
    return desktopExposed || (remainingReasons & ~sc<uint32_t>(INPUT_BLOCK_SPATIAL_OUTSIDE_VIEW)) != 0;
}
...
void CWindow::setInputBlocked(eWindowInputBlockReason reason, bool blocked) {
    ...
    // Clearing an unrelated reason must not revoke focus because spatial
    // visibility still blocks pointer input. Other remaining reasons still do.
    if (inputBlockRevokesFocus(m_inputBlockReasons, Luminophore::spatialRuntime()->desktopExposed()) && Desktop::focusState()->window() == m_self)
...
bool CWindow::isInputBlocked() const {
    return m_inputBlockReasons != INPUT_BLOCK_NONE || Luminophore::spatialRuntime()->desktopExposed();
}
...
bool CWindow::acceptsInput() const {
    return !isDesktopSuppressed() && !isInputBlocked();
}
...
bool CWindow::isBlockedByFullscreen() const {
    if (Luminophore::spatialRuntime()->isBoardRoot(m_self.lock()))
        return false;
...
bool CWindow::targetVisible() const {
    return !isDesktopSuppressed() && m_isMapped && m_wlSurface && m_wlSurface->resource() && visibleByAlphaGoal();
}
...
void CWindow::updateDecorationValues() {
    ...
    if (m_self == Desktop::focusState()->window()) {
        setBorderColor(m_ruleApplicator->activeBorderColor().valueOr(*ACTIVECOL));
    } else {
        setBorderColor(m_ruleApplicator->inactiveBorderColor().valueOr(*INACTIVECOL));
    }
...
void CWindow::mapWindow() {
    ...
    // Only an explicit internal service launch may select a non-base container.
    const auto environment = getEnv();
    if (const auto it = environment.find("LUMINOPHORE_SERVICE_LAUNCH_TOKEN"); it != environment.end()) {
        if (const auto token = g_pTokenManager->getToken(it->second)) {
            if (const auto service = std::any_cast<SServiceWorkspaceToken>(&token->m_data)) {
                if (const auto target = service->workspace.lock(); target && target->role() == WORKSPACE_ROLE_SERVICE) {
                    requestedWorkspace = target->getConfigName();
    ...
    // Capture the swallowee before registering the new layout target.
...
void CWindow::unmapWindow() {
    ...
    if (wasLastWindow) {
        ...
        if (candidate != Desktop::focusState()->window() && candidate) {
            ...
            if (*PEXITRETAINSFS && IS_CURRENT_WINDOW_FS)
...
void CWindow::onXDGMoveRequest(const SXDGToplevelMoveRequest&) {
    if (!m_isMapped || isDesktopSuppressed() || g_layoutManager->dragController()->target())
        return;
