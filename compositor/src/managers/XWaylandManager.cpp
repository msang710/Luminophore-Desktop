...
bool CHyprXWaylandManager::shouldBeFloated(PHLWINDOW pWindow, bool pending) {
    if (pWindow->m_isX11) {
        // checkBorders() runs before initial placement. Reuse its native role
        // classification so auxiliary surfaces remain visible outside the board.
        if (pWindow->m_X11DoesntWantBorders) {
            pWindow->m_noInitialFocus = pWindow->m_noInitialFocus || !pWindow->m_xwaylandSurface->wantsFocus();
            return true;
        }
