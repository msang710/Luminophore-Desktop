...
class CInputManager {
  public:
    CInputManager();
    ~CInputManager();
    void               settleSpatialDragFocus();
    uint32_t           spatialDragPressTime() const;
    Vector2D           spatialDragPressPosition() const;
  ...
  private:
    ...
    uint32_t            m_spatialDragPressTime = 0;
    Vector2D            m_spatialDragPressPosition;
    ...
    eBorderIconDirection            borderIconDirectionForWindow(PHLWINDOW, const Vector2D&) const;
