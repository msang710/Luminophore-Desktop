...
void CKeybindManager::updateXKBTranslationState() {
    if (Config::mgr()->type() == Config::CONFIG_LUMINOPHORE) {
        const auto           get  = [](const std::string& key) { return Config::mgr()->getDeviceString("", key, "input:" + key); };
        const auto           file = get("kb_file"), snapshot = get("kb_snapshot");
        const auto           rulesName = get("kb_rules"), model = get("kb_model"), layout = get("kb_layout"), variant = get("kb_variant"), options = get("kb_options");
        const xkb_rule_names rules{rulesName.c_str(), model.c_str(), layout.c_str(), variant.c_str(), options.c_str()};
        const auto context = std::unique_ptr<xkb_context, decltype(&xkb_context_unref)>(xkb_context_new(snapshot.empty() ? XKB_CONTEXT_NO_FLAGS : XKB_CONTEXT_NO_DEFAULT_INCLUDES),
                                                                                        xkb_context_unref);
        if (!context || (!file.empty() && !snapshot.starts_with(file + "\n")))
            throw std::runtime_error("keybind keymap snapshot unavailable");
        const auto keymap = std::unique_ptr<xkb_keymap, decltype(&xkb_keymap_unref)>(
            snapshot.empty() ? xkb_keymap_new_from_names2(context.get(), &rules, XKB_KEYMAP_FORMAT_TEXT_V2, XKB_KEYMAP_COMPILE_NO_FLAGS) :
                               xkb_keymap_new_from_string(context.get(), snapshot.c_str() + file.size() + 1, XKB_KEYMAP_FORMAT_TEXT_V2, XKB_KEYMAP_COMPILE_NO_FLAGS),
            xkb_keymap_unref);
        if (!keymap)
            throw std::runtime_error("keybind keymap compilation failed");
        const auto state = xkb_state_new(keymap.get());
        if (!state)
            throw std::runtime_error("keybind translation state unavailable");
        if (m_xkbTranslationState)
            xkb_state_unref(m_xkbTranslationState);
        m_xkbTranslationState = state;
        return;
    }
    if (m_xkbTranslationState) {
        xkb_state_unref(m_xkbTranslationState);
...
bool CKeybindManager::ensureMouseBindState(bool cancelled) {
...
bool CKeybindManager::onKeyEvent(std::any event, SP<IKeyboard> pKeyboard) {
    ...
    if ((keysym == XKB_KEY_Escape || internalKeysym == XKB_KEY_Escape) && Luminophore::livePipController()->escape(e.state == WL_KEYBOARD_KEY_STATE_PRESSED))
        return false;
    ...
    bool       mouseBindWasActive = ensureMouseBindState(true);
...
bool CKeybindManager::onMouseEvent(const IPointer::SButtonEvent& e, SP<IPointer> mouse) {
    ...
    bool       mouseBindWasActive = ensureMouseBindState(e.state != WL_POINTER_BUTTON_STATE_RELEASED);
