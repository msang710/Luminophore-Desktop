...
void CMonitor::onConnect(bool noRule) {
    ...
    m_listeners.presented = m_output->events.present.listen([this](const Aquamarine::IOutput::SPresentEvent& event) {
        ...
        const auto PRESENTED = Time::fromTimespec(&ts);
#ifdef LUMINOPHORE_EFFECTS
        Luminophore::monitorTransaction()->presented(m_self.lock());
#endif
        m_events.presented.emit(PRESENTED);
    });
    ...
    for (auto const& ws : State::workspaceState()->workspacesCopy()) {
        ...
        if (RETURNING || RECOVERY) {
            State::workspacePlacementController()->recoverWorkspaceOnMonitor(ws, m_self.lock());
...
void CMonitor::onDisconnect(bool destroy) {
    ...
    if (BACKUPMON) {
        ...
        for (auto const& w : wspToMove) {
            State::workspacePlacementController()->recoverWorkspaceOnMonitor(w, BACKUPMON);
...
void CMonitor::setupDefaultWS(const Config::CMonitorRule& monitorRule) {
    const auto base = State::workspaceState()->ensureBaseForMonitor(m_self.lock());
    RASSERT(base, "Monitor must own a base desktop");
    m_activeWorkspace = base;
    base->m_alpha->setValueAndWarp(1.F);
    base->m_events.activeChanged.emit();
    base->m_visible     = true;
    base->m_lastMonitor = "";
}
...
void CMonitor::changeWorkspace(const PHLWORKSPACE& pWorkspace, bool internal, bool noMouseMove, bool noFocus) {
    ...
    if (pWorkspace->m_isSpecialWorkspace) {
        if (m_activeSpecialWorkspace != pWorkspace) {
            Log::logger->log(Log::DEBUG, "change hidden service workspace to id {}", pWorkspace->m_id);
...
bool CMonitor::canAttemptDirectScanoutFast() const {
    if (Luminophore::spatialRuntime()->requiresComposition(m_self.lock()))
        return false;
#ifdef LUMINOPHORE_EFFECTS
    if (Luminophore::compositionPolicy()->blocksDirectScanout(m_self.lock()))
        return false;
#endif
