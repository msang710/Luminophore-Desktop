...
void IKeyboard::setKeymap(const SStringRuleNames& rules) {
    ...
    const auto CONTEXT = xkb_context_new(m_xkbSnapshot.empty() ? XKB_CONTEXT_NO_FLAGS : XKB_CONTEXT_NO_DEFAULT_INCLUDES);
    ...
    std::unique_ptr<xkb_keymap, decltype(&xkb_keymap_unref)> imported(nullptr, xkb_keymap_unref);
    if (!m_xkbSnapshot.empty()) {
        const auto prefix = m_xkbFilePath + "\n";
        if (m_xkbSnapshot.starts_with(prefix))
            imported.reset(xkb_keymap_new_from_string(CONTEXT, m_xkbSnapshot.c_str() + prefix.size(), XKB_KEYMAP_FORMAT_TEXT_V2, XKB_KEYMAP_COMPILE_NO_FLAGS));
        if (!imported) {
            Log::logger->log(Log::ERR, "Imported keyboard map invalid; preserving current map");
            m_xkbSnapshot.clear(); // Runtime verification must not report the rejected map as applied.
            xkb_context_unref(CONTEXT);
            return;
        }
    }
    clearManuallyAllocd();
    m_xkbKeymap = imported.release();
    ...
    if (!m_xkbFilePath.empty() && m_xkbSnapshot.empty() && Config::mgr()->type() != Config::CONFIG_LUMINOPHORE) {
