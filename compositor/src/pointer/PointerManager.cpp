...
void CPointerManager::attachPointer(SP<IPointer> pointer) {
    ...
    listener->motion  = pointer->m_pointerEvents.motion.listen([weak = WP<IPointer>(pointer)](const IPointer::SMotionEvent& event) {
        g_pInputManager->onMouseMoved(event);

        PROTO::idle->onActivity();
#ifdef LUMINOPHORE_EFFECTS
        if (const auto DEVICE = weak.lock(); DEVICE && !DEVICE->isVirtual())
    ...
    listener->motionAbsolute = pointer->m_pointerEvents.motionAbsolute.listen([weak = WP<IPointer>(pointer)](const IPointer::SMotionAbsoluteEvent& event) {
...
void CPointerManager::attachTouch(SP<ITouch> touch) {
    ...
    listener->down = touch->m_touchEvents.down.listen([weak = WP<ITouch>(touch)](const ITouch::SDownEvent& event) {
