...
SWorkspaceIDName getWorkspaceIDNameFromString(const std::string& in) {
    // User spaces are the spatial board. Only internal base/service identities remain.
    const bool service = in == "special:luminophore-spotify";
    if (!service && !in.starts_with("name:luminophore-base-"))
        return {WORKSPACE_INVALID, ""};
    const auto name = service ? in : in.substr(5);
    if (!service && name.size() == std::string_view{"luminophore-base-"}.size())
        return {WORKSPACE_INVALID, ""};
    const auto existing = State::workspaceState()->query().name(name).run();
    return {existing ? existing->m_id : (service ? State::workspaceState()->newSpecialID() : State::workspaceState()->nextAvailableNamedWorkspace()), name, true};
}
...
std::string getSystemLibraryVersion(const std::string& name) {
    // The release loads private DSOs. Host pkg-config files describe another
    // installation, and build versions alone do not prove the loaded ABI.
    if (const auto* ROOT = std::getenv("LUMINOPHORE_RELEASE_ROOT"); ROOT && *ROOT)
        return "unknown (private release; host pkg-config excluded)";
