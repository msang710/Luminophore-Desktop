...
void CHyprBorderDecoration::draw(PHLMONITOR pMonitor, float const& a) {
    ...
    const auto dragBox   = Luminophore::spatialGrabController()->visualBox(m_window.lock());
    CBox       windowBox = dragBox ?
        dragBox->copy().translate(-pMonitor->m_position).scale(pMonitor->m_scale).round() :
        assignedBoxGlobal().translate(-pMonitor->m_position + m_window->m_floatingOffset).expand(-m_window->getRealBorderSize()).scale(pMonitor->m_scale).round();
