...
void IElementRenderer::drawSurface(WP<CSurfacePassElement> element, const CRegion& damage) {
    ...
    const float ALPHA         = m_data.alpha * m_data.fadeAlpha * (PSURFACE && !m_data.sourceSampleBox ? PSURFACE->m_alphaModifier : 1.F);
    const float OVERALL_ALPHA = PSURFACE && !m_data.sourceSampleBox ? PSURFACE->m_overallOpacity : 1.F;
    const bool  BLUR          = m_data.blur && (m_data.forceBlurRegion || !TEXTURE->m_opaque || ALPHA < 1.F || OVERALL_ALPHA < 1.F);
    ...
    const bool MISALIGNEDFSV1 = !m_data.sourceSampleBox && std::floor(m_data.pMonitor->m_scale) != m_data.pMonitor->m_scale /* Fractional */ &&
        m_data.surface->m_current.scale == 1 /* fs protocol */ && windowBox.size() != m_data.surface->m_current.bufferSize /* misaligned */ &&
    ...
    if (m_data.sourceSampleUV) {
        m_renderData.primarySurfaceUVTopLeft     = m_data.sourceSampleUV->pos();
        m_renderData.primarySurfaceUVBottomRight = m_data.sourceSampleUV->pos() + m_data.sourceSampleUV->size();
    } else
    ...
    if (m_data.spatialClip) {
        if (clipRegion.empty())
            clipRegion = *m_data.spatialClip;
        else
            clipRegion.intersect(*m_data.spatialClip);
    }
    if (cancelRender || (m_data.spatialClip && clipRegion.empty())) {
        element->discard();
        return;
    }
    ...
    if (m_data.surfaceCounter == 0 && !m_data.popup) {
        if (BLUR)
            drawElement(makeShared<CTexPassElement>(CTexPassElement::SRenderData{
                            ...
                            .blur                  = true,
                            .forceBlurBlend        = m_data.forceBlurRegion,
                            .blockBlurOptimization = m_data.blockBlurOptimization,
                            .forceBlurRegion       = m_data.mainSurface && m_data.forceBlurRegion,
                            .blurParameters        = m_data.blurParameters,
                            .blurRegion            = m_data.blurRegion,
                            .allowCustomUV         = true,
...
void IElementRenderer::drawTex(WP<CTexPassElement> element, const CRegion& damage) {
    ...
    if (element->m_data.blur) {
        ...
        if (element->m_data.forceBlurRegion)
            inverseOpaque = element->m_data.blurRegion;
        else if (element->m_data.a >= 1.f && surface && std::round(surface->m_current.size.x * m_renderData.pMonitor->m_scale) == box.w &&
        ...
        element->m_data.blockBlurOptimization = element->m_data.blurParameters.has_value() || element->m_data.blockBlurOptimization.value_or(false) ||
        ...
        if (element->m_data.blockBlurOptimization.value_or(false)) {
            ...
            element->m_data.blurredBG = g_pHyprRenderer->blurMainFramebuffer(element->m_data.a, &inverseOpaque, element->m_data.blurParameters);
        } else
