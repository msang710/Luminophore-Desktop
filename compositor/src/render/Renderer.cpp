...
#include "luminophore/LuminophoreWindowEffect.hpp"
#include "../luminophore/LuminophoreMonitorTransaction.hpp"
...
#ifdef LUMINOPHORE_EFFECTS
    m_luminophoreWindowEffect = makeUnique<CLuminophoreWindowEffect>();
#endif
...
IHyprRenderer::~IHyprRenderer() {
#ifdef LUMINOPHORE_EFFECTS
    m_luminophoreWindowEffect.reset();
#endif
    if (m_cursorTicker)
...
bool IHyprRenderer::shouldRenderWindow(PHLWINDOW pWindow, PHLMONITOR pMonitor) {
    if (!validMapped(pWindow) || !pMonitor)
        return false;

    if (Luminophore::spatialRuntime()->isBoardRoot(pWindow))
        return !pWindow->isDesktopSuppressed() && !Luminophore::spatialRuntime()->regionsFor(pWindow, pMonitor).empty();
    ...
    if (PWINDOWWORKSPACE && PWINDOWWORKSPACE->m_monitor == pMonitor) {
        if (PWINDOWWORKSPACE->m_renderOffset->isBeingAnimated() || PWINDOWWORKSPACE->m_alpha->isBeingAnimated())
            return true;
...
bool IHyprRenderer::shouldRenderWindow(PHLWINDOW pWindow) {
    ...
    if (Luminophore::spatialRuntime()->isBoardRoot(pWindow)) {
        const auto presentation = Luminophore::spatialRuntime()->presentationFor(pWindow);
        return presentation && presentation->visible && !pWindow->isDesktopSuppressed();
    }
...
void IHyprRenderer::renderWorkspaceWindowsFullscreen(PHLMONITOR pMonitor, PHLWORKSPACE pWorkspace, const Time::steady_tp& time) {
    ...
    for (auto const& w : Desktop::windowState()->windows()) {
        ...
        if (w->m_workspace != pWorkspace || !Fullscreen::controller()->isFullscreen(w)) {
            if (!(PWORKSPACE && (PWORKSPACE->m_renderOffset->isBeingAnimated() || PWORKSPACE->m_alpha->isBeingAnimated())))
                continue;
...
void IHyprRenderer::renderWorkspaceWindows(PHLMONITOR pMonitor, PHLWORKSPACE pWorkspace, const Time::steady_tp& time) {
    ...
    for (auto const& w : Desktop::windowState()->windows()) {
        const bool isNotRenderable = w->isDesktopSuppressed() || !w->m_isMapped;
...
void IHyprRenderer::renderWindow(PHLWINDOW pWindow, PHLMONITOR pMonitor, const Time::steady_tp& time, bool decorate, eRenderPassMode mode, bool ignorePosition, bool standalone) {
    if (pWindow->isDesktopSuppressed() && !standalone)
        return;
    ...
    if (!standalone && !ignorePosition)
        if (const auto visual = Luminophore::spatialGrabController()->visualBox(pWindow))
            textureBox = *visual;
    ...
    if (Luminophore::spatialRuntime()->isWideWindow(pWindow)) {
    ...
    if (pWindow->m_ruleApplicator->opaque().valueOrDefault())
        renderdata.alpha = 1.f;
    if (!standalone)
        renderdata.alpha *= Luminophore::spatialGrabController()->visualAlpha(pWindow);
    ...
    if (!standalone && Luminophore::spatialRuntime()->isBoardRoot(pWindow)) {
        CRegion clip;
        for (auto box : Luminophore::spatialRuntime()->regionsFor(pWindow, pMonitor))
            clip.add(box.translate(-pMonitor->m_position).scale(pMonitor->m_scale).round());
        renderdata.spatialClip = clip;
    }

    if (standalone && Luminophore::spatialRuntime()->isBoardRoot(pWindow)) {
        CRegion    clip;
        const auto entry = Luminophore::spatialRuntime()->presentationFor(pWindow);
        // Existing standalone captures show the spatial presentation. Live PiP
        // must sample its selected surface directly, not call this capture path.
        const auto presentation = entry ? entry->presentationBox() : std::nullopt;
        if (presentation) {
            for (const auto& f : entry->fragments) {
                CBox box{(f.box.x - presentation->x) * renderdata.w / presentation->width, (f.box.y - presentation->y) * renderdata.h / presentation->height,
                         f.box.width * renderdata.w / presentation->width, f.box.height * renderdata.h / presentation->height};
                clip.add(box.scale(pMonitor->m_scale).round());
            }
        }
        renderdata.spatialClip = clip;
    }
...
#ifdef LUMINOPHORE_EFFECTS
        if (!standalone && Luminophore::compositionPolicy()->showsWindowEffect(pWindow))
            m_luminophoreWindowEffect->enqueue(pWindow, pMonitor, renderdata.pos, {renderdata.w, renderdata.h}, fullAlpha);
#endif
    ...
    if (mode == RENDER_PASS_ALL || mode == RENDER_PASS_POPUP) {
        if (!pWindow->m_isX11) {
            ...
            renderdata.spatialClip.reset(); // Popup surfaces have their own geometry and input region.
...
void IHyprRenderer::renderLayer(PHLLS pLayer, PHLMONITOR pMonitor, const Time::steady_tp& time, bool popups, bool lockscreen, const Vector2D& offset, float opacity) {
    if (!pLayer)
        return;

#ifdef LUMINOPHORE_EFFECTS
    // Asset-only surfaces are displayed exclusively by the native spatial grab.
    if (pLayer->m_namespace.starts_with("luminophore-shell-spatial-badge-"))
        return;
#endif
    ...
    const auto                       REALPOS = pLayer->position(Desktop::View::IGeometric::GEOMETRIC_CURRENT) + offset;
    ...
    renderdata.fadeAlpha                        = pLayer->alpha()[LS_ALPHA_FADE]->value() * std::clamp(opacity, 0.F, 1.F);
...
#ifdef LUMINOPHORE_EFFECTS
    if (const auto SNAPSHOT = Luminophore::shellProjection()->snapshotFor(pLayer, pMonitor)) {
        renderdata.blur = SNAPSHOT->visualBundle.blurEnabled && !SNAPSHOT->blurBox.empty();
        if (pLayer->m_namespace == "luminophore-shell-spatial-editor") {
            // Sample the application backdrop before any GTK surface is drawn.
            // Grid and glow subsurfaces must retain their own premultiplied alpha.
            if (renderdata.blur && !popups) {
                CRectPassElement::SRectData backdrop;
                backdrop.box            = SNAPSHOT->blurBox.copy().translate(-pMonitor->m_position).scale(pMonitor->m_scale).round();
                backdrop.color          = CHyprColor(0, 0, 0, 0);
                backdrop.blur           = true;
                backdrop.blurA          = renderdata.fadeAlpha;
                backdrop.blurParameters = SNAPSHOT->visualBundle.blurParameters();
                m_renderPass.add(makeUnique<CRectPassElement>(backdrop));
            }
            renderdata.blur = false;
        } else if (renderdata.blur) {
            // Projected LUMINOPHORE panels own their backdrop blur contract.  Do not
            // depend on a layer rule being re-applied after the first commit:
            // cold-start panels must enter the blur pass on their first frame.
            renderdata.blur                  = true;
            renderdata.forceBlurRegion       = true;
            renderdata.blockBlurOptimization = true;
            renderdata.blurParameters        = SNAPSHOT->visualBundle.blurParameters();
            renderdata.blurRegion            = SNAPSHOT->blurBox.copy().translate(-REALPOS);
        }
    }
#endif
...
void IHyprRenderer::renderAllClientsForWorkspace(PHLMONITOR pMonitor, PHLWORKSPACE pWorkspace, const Time::steady_tp& time, const Vector2D& translate, const float& scale) {
    ...
    const auto renderPip = [&] {
        if (!RENDERMODIFDATA.modifs.empty())
            m_renderPass.add(makeUnique<CRendererHintsPassElement>(CRendererHintsPassElement::SData{SRenderModifData{}}));
        Luminophore::livePipController()->render(pMonitor, time);
        if (!RENDERMODIFDATA.modifs.empty())
            m_renderPass.add(makeUnique<CRendererHintsPassElement>(CRendererHintsPassElement::SData{RENDERMODIFDATA}));
    };
    ...
    if UNLIKELY (!pWorkspace) {
        ...
        if (!g_pSessionLockManager->isSessionLocked())
            Luminophore::spatialGrabController()->renderBadge(pMonitor, time);
#endif
...
#ifdef LUMINOPHORE_EFFECTS
            if (const auto SNAPSHOT = Luminophore::shellProjection()->snapshotFor(SURFACE, pMonitor)) {
                if (SNAPSHOT->plane == Luminophore::SHELL_PROJECTION_OVERLAY)
                    continue;
                Luminophore::shellProjection()->enqueueBloom(SNAPSHOT);
                renderLayer(SURFACE, pMonitor, time, false, false, SNAPSHOT->offset, SNAPSHOT->opacity);
                continue;
            }
#endif
            renderLayer(SURFACE, pMonitor, time);
        }
        renderFadeouts(pMonitor, Desktop::FADEOUT_PLANE_LAYER_BOTTOM);
    }
    ...
    if UNLIKELY /* subjective? */ (Fullscreen::controller()->hasFullscreen(pWorkspace) && !(pWorkspace->m_space && pWorkspace->m_space->usesLuminophoreSpatialGeometry()))
    ...
    // pinned always above
    for (auto const& w : Desktop::windowState()->windows()) {
        if (w->isDesktopSuppressed() || !w->m_isMapped)
            continue;
...
#ifdef LUMINOPHORE_EFFECTS
    renderLuminophoreShellPlane(pMonitor, time);
#endif
...
void IHyprRenderer::renderLuminophoreShellPlane(PHLMONITOR pMonitor, const Time::steady_tp& time) {
#ifdef LUMINOPHORE_EFFECTS
    TRACY_GPU_ZONE("RenderLuminophoreShellPlane");
    for (const auto& SNAPSHOT : Luminophore::shellProjection()->renderList(pMonitor, Luminophore::SHELL_PROJECTION_OVERLAY)) {
...
std::string IHyprRenderer::resolveAssetPath(const std::string& filename) {
    ...
    for (auto& e : ASSET_PATHS) {
        std::string     p = std::string{e} + "/luminophore/" + filename;
        std::error_code ec;
...
SP<ITexture> IHyprRenderer::blurMainFramebuffer(float a, CRegion* originalDamage, const std::optional<SBlurParameters>& parameters) {
    ...
    return blurFramebuffer(m_renderData.currentFB, a, originalDamage, parameters);
}
...
void IHyprRenderer::renderMonitor(PHLMONITOR pMonitor, bool commit) {
    ...
    Luminophore::spatialRuntime()->advanceMotion();

#ifdef LUMINOPHORE_EFFECTS
    Luminophore::monitorTransaction()->beginFrame(pMonitor);
#endif
    ...
    if (canAttemptDirectScanout) {
        // Rehome PiPs before scanout eligibility; a removed output must not
        // strand them behind a scanout-only render loop.
        Luminophore::livePipController()->reconcileOutputs();
        if (!Luminophore::livePipController()->occupies(pMonitor) && pMonitor->attemptDirectScanout()) {
            if (!pMonitor->needsACopyFB())
...
void IHyprRenderer::damageWindow(PHLWINDOW pWindow, bool forceFull) {
    ...
    const bool spatialRoot = Luminophore::spatialRuntime()->isBoardRoot(pWindow);
    for (auto const& m : State::monitorState()->monitors()) {
        const bool fragmentVisible = spatialRoot && !Luminophore::spatialRuntime()->regionsFor(pWindow, m).empty();
        if (forceFull || fragmentVisible || (!spatialRoot && shouldRenderWindow(pWindow, m))) { // only damage if window is rendered on monitor
            CBox fixedDamageBox = {windowBox.x - m->m_position.x, windowBox.y - m->m_position.y, windowBox.width, windowBox.height};
            fixedDamageBox      = fixedDamageBox.intersection(CBox{{}, m->m_size});
            if (fixedDamageBox.empty())
                continue;
