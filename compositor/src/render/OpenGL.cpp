...
void CHyprOpenGLImpl::renderRectWithBlurInternal(const CBox& box, const CHyprColor& col, const SRectRenderData& data) {
    ...
    auto blurredBG =
        data.xray ? g_pHyprRenderer->m_renderData.pMonitor->resources()->m_blurFB->getTexture() : g_pHyprRenderer->blurMainFramebuffer(data.blurA, &damage, data.blurParameters);
...
SP<IFramebuffer> CHyprOpenGLImpl::blurFramebufferWithDamage(float a, CRegion* originalDamage, CGLFramebuffer& source, const std::optional<SBlurParameters>& parameters) {
    ...
    const auto  BLUR_PASSES = std::clamp<int64_t>(parameters ? parameters->passes : *PBLURPASSES, 1, 8);
    const auto  BLUR_SIZE   = std::clamp<int64_t>(parameters ? parameters->size : *PBLURSIZE, 1, 40);
    ...
    damage.transform(Math::wlTransformToHyprutils(Math::invertTransform(m_renderData.pMonitor->m_transform)), m_renderData.pMonitor->m_transformedSize.x,
                     m_renderData.pMonitor->m_transformedSize.y);
    damage.expand(BLUR_SIZE * pow(2, BLUR_PASSES));
    ...
    // TODO: make this a part of the first pass maybe to save on a drawcall?
    {
        ...
        shader->setUniformMatrix3fv(SHADER_PROJ, 1, GL_TRUE, glMatrix.getMatrix());
        shader->setUniformFloat(SHADER_CONTRAST, (parameters ? parameters->contrast : *PBLURCONTRAST));
        shader->setUniformFloat(SHADER_BRIGHTNESS, (parameters ? parameters->brightness : *PBLURBRIGHTNESS));
        shader->setUniformInt(SHADER_TEX, 0);
    ...
    auto drawPass = [&](WP<CShader> shader, ePreparedFragmentShader frag, CRegion* pDamage) {
        ...
        // prep two shaders
        shader->setUniformMatrix3fv(SHADER_PROJ, 1, GL_TRUE, glMatrix.getMatrix());
        shader->setUniformFloat(SHADER_RADIUS, BLUR_SIZE * a); // this makes the blursize change with a
        if (frag == SH_FRAG_BLUR1) {
            ...
            shader->setUniformFloat(SHADER_VIBRANCY, (parameters ? parameters->vibrancy : *PBLURVIBRANCY));
            shader->setUniformFloat(SHADER_VIBRANCY_DARKNESS, (parameters ? parameters->vibrancyDarkness : *PBLURVIBRANCYDARKNESS));
        } else
            shader->setUniformFloat2(SHADER_HALFPIXEL, 0.5f / (m_renderData.pMonitor->m_pixelSize.x * 2.f), 0.5f / (m_renderData.pMonitor->m_pixelSize.y * 2.f));
        shader->setUniformInt(SHADER_TEX, 0);
    ...
    // finalize the image
    {
        ...
        shader->setUniformMatrix3fv(SHADER_PROJ, 1, GL_TRUE, glMatrix.getMatrix());
        shader->setUniformFloat(SHADER_NOISE, (parameters ? parameters->noise : *PBLURNOISE));
        shader->setUniformFloat(SHADER_BRIGHTNESS, (parameters ? parameters->brightness : *PBLURBRIGHTNESS));
...
void CHyprOpenGLImpl::renderTextureWithBlurInternal(SP<ITexture> tex, const CBox& box, const STextureRenderData& data) {
    ...
    renderTextureInternal(tex, box,
                          STextureRenderData{
                              .blur           = *PBLEND || data.forceBlurBlend,
                              .forceBlurBlend = data.forceBlurBlend,
                              ...
                              .round          = data.round,
                              .roundingPower  = data.roundingPower,
                              .discardActive  = (*PBLEND || data.forceBlurBlend) && NEEDS_STENCIL,
                              .allowCustomUV  = true,
                              .allowDim       = true,
                              .noAA           = false,
                              .wrapX          = data.wrapX,
