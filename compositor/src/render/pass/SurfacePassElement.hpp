#pragma once
#include "PassElement.hpp"
#include "../BlurParameters.hpp"
#include "TexPassElement.hpp"
#include <optional>
#include "../../helpers/time/Time.hpp"

class CWLSurfaceResource;
namespace Render {
    class ITexture;
}
class CSyncTimeline;

class CSurfacePassElement : public IPassElement {
  public:
    struct SRenderData {
        PHLMONITORREF                          pMonitor;
        Time::steady_tp                        when = Time::steadyNow();
        Vector2D                               pos, localPos;

        void*                                  data        = nullptr;
        SP<CWLSurfaceResource>                 surface     = nullptr;
        SP<Render::ITexture>                   texture     = nullptr;
        bool                                   mainSurface = true;
        double                                 w = 0, h = 0;
        int                                    rounding      = 0;
        bool                                   dontRound     = true;
        float                                  roundingPower = 2.0F;
        bool                                   decorate      = false;
        float                                  alpha = 1.F, fadeAlpha = 1.F;
        bool                                   blur                  = false;
        bool                                   blockBlurOptimization = false;
        bool                                   forceBlurRegion       = false;
        std::optional<Render::SBlurParameters> blurParameters;
        CBox                                   blurRegion = {};

        uint8_t                                wrapX = WRAP_CLAMP_TO_EDGE;
        uint8_t                                wrapY = WRAP_CLAMP_TO_EDGE;

        // only for windows, not popups
        bool squishOversized = true;

        // for calculating UV
        PHLWINDOW pWindow;
        PHLLS     pLS;

        bool      popup = false;

        // counts how many surfaces this pass has rendered
        int surfaceCounter = 0;

        // Explicit source sampling bypasses desktop geometry, opacity and visibility.
        std::optional<CBox>    sourceSampleBox;
        std::optional<CBox>    sourceSampleUV;
        std::optional<CRegion> spatialClip;
        CBox                   clipBox = {}; // scaled coordinates

        uint8_t                discardMode    = DISCARD_OPAQUE;
        float                  discardOpacity = 0.f;

        bool                   useNearestNeighbor = false;

        bool                   flipEndFrame = false;
    };

    CSurfacePassElement(const SRenderData& data);
    virtual ~CSurfacePassElement() = default;

    std::optional<float>        blurDamageRadius() override;
    virtual bool                needsLiveBlur();
    virtual bool                needsPrecomputeBlur();
    virtual std::optional<CBox> boundingBox();
    virtual CRegion             opaqueRegion();
    virtual void                discard();
    CRegion                     visibleRegion(bool& cancel);

    virtual const char*         passName() {
        return "CSurfacePassElement";
    }

    virtual ePassElementType type() {
        return EK_SURFACE;
    };

    SRenderData m_data;

    CBox        getTexBox();

  private:
    bool m_texBoxCached = false;
    CBox m_cachedTexBox = {};
};
