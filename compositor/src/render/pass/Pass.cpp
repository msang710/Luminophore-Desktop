...
float CRenderPass::oneBlurRadius() {
    ...
    float       radius = std::clamp(*PBLURSIZE, sc<int64_t>(1), sc<int64_t>(40)) * pow(2, BLUR_PASSES);
    // Keep backdrop pixels needed by an explicit per-pass kernel even when
    // the global blur kernel is smaller.
    for (const auto& pass : m_passElements) {
        if (const auto explicitRadius = pass.element->blurDamageRadius())
            radius = std::max(radius, *explicitRadius);
    }
    return radius;
}
