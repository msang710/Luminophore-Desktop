...
CBox CSurfacePassElement::getTexBox() {
    if (m_data.sourceSampleBox)
        return *m_data.sourceSampleBox;
    if (m_texBoxCached)
...
bool CSurfacePassElement::needsLiveBlur() {
    ...
    const bool  BLUR  = m_data.blur && (m_data.forceBlurRegion || !m_data.texture || !m_data.texture->m_opaque || ALPHA < 1.F);

    if (m_data.blurParameters)
        return BLUR;
...
CRegion CSurfacePassElement::opaqueRegion() {
    // Conservative occlusion avoids hiding another owner inside a concave window.
    if (m_data.spatialClip || m_data.sourceSampleBox)
        return {};
    auto        PSURFACE = Desktop::View::CWLSurface::fromResource(m_data.surface);
...
std::optional<float> CSurfacePassElement::blurDamageRadius() {
    if (!m_data.blur || !m_data.blurParameters)
        return std::nullopt;
    return std::clamp(m_data.blurParameters->size, 1, 40) * std::pow(2.F, std::clamp(m_data.blurParameters->passes, 1, 8));
}
