...
std::string CShaderLoader::loadShader(const std::string& filename) {
    ...
    if (home.has_value()) {
        const auto src = NFsUtils::readFileAsString(home.value() + "/luminophore/shaders/" + filename);
        if (src.has_value())
            return src.value();
    }
    for (auto& e : ASSET_PATHS) {
        const auto src = NFsUtils::readFileAsString(std::string{e} + "/luminophore/shaders/" + filename);
        if (src.has_value())
            return src.value();
    }
