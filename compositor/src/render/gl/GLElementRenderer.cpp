...
void CGLElementRenderer::draw(WP<CRectPassElement> element, const CRegion& damage) {
    ...
    if (m_data.color.a == 1.F || !m_data.blur)
        ...
        g_pHyprOpenGL->renderRect(
            m_data.box, m_data.color,
            {.round = m_data.round, .roundingPower = m_data.roundingPower, .blur = true, .blurA = m_data.blurA, .xray = m_data.xray, .blurParameters = m_data.blurParameters});
};
