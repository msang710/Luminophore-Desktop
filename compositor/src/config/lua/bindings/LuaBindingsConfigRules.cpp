...
namespace {
    ...
    inline constexpr SFieldDesc DEVICE_FIELDS[] = {
        ...
        {"kb_snapshot", []() -> ILuaConfigValue* { return new CLuaConfigString(STRVAL_EMPTY); }},
...
static int hlWorkspaceRule(lua_State* L) {
    ...
    if (wsId == WORKSPACE_INVALID) {
        self->addError("hl.workspace_rule: only internal base/service workspace rules are supported");
        return 0;
    }
