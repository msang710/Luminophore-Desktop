...
void CConfigManager::setDeviceSettings(const Luminophore::Settings::DeviceSettings& devices) {
    Luminophore::Settings::Snapshot globals;
    for (const auto* field : {"region_position", "region_size", "active_area_position", "active_area_size"}) {
        const auto value                               = getDeviceVec("", field, std::string("input:tablet:") + field);
        globals[std::string("tablet.") + field + "_x"] = value.x;
        globals[std::string("tablet.") + field + "_y"] = value.y;
    }
    std::unordered_map<std::string, SDeviceConfig> candidate;
    for (const auto& [name, fields] : devices) {
        const auto normalized = normalizeDeviceName(name);
        if (candidate.contains(normalized))
            throw std::runtime_error("duplicate normalized device name: " + name);
        candidate[normalized].values = makeDeviceValues(fields, globals);
    }
    m_deviceConfigs.swap(candidate);
}
Luminophore::Settings::DeviceSettings CConfigManager::deviceSettings() const {
    Luminophore::Settings::DeviceSettings result;
    for (const auto& [name, device] : m_deviceConfigs)
        result[name] = readDeviceValues(device.values);
    return result;
}

void CConfigManager::setValueExplicit(const std::string& name, bool explicitValue) {
    const auto key   = luaConfigValueName(name);
    const auto value = m_configValues.find(key);
    if (value == m_configValues.end())
        throw std::runtime_error("unknown Lua config value: " + name);
    value->second->setExplicit(explicitValue);
}
