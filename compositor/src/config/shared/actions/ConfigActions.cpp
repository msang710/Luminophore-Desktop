...
static std::optional<eLuminophoreSpatialDirection> spatialDirection(Math::eDirection dir) {
    switch (dir) {
...
ActionResult Actions::occupyOutput(Luminophore::eOccupyOutputAction action, const std::string& serial, std::optional<PHLWINDOW> w) {
    auto window = xtract(w);
    if (!window)
        return {};

    Luminophore::occupyOutputController()->apply(window, action, serial);
    return {};
}

ActionResult Actions::minimizeWindow(std::optional<PHLWINDOW> w) {
    auto window = xtract(w);
    return Luminophore::windowGestureController()->minimize(window) ? ActionResult{} : actionError("LUMINOPHORE minimize rejected", eActionErrorLevel::WARNING);
}

ActionResult Actions::restoreWindow(std::optional<PHLWINDOW> w) {
    auto window = xtract(w);
    return Luminophore::windowGestureController()->restore(window) ? ActionResult{} : actionError("LUMINOPHORE restore rejected", eActionErrorLevel::WARNING);
}

ActionResult Actions::shellProjection(const std::string& surfaceNamespace, const std::string& generation, uint64_t revision, uint64_t contentRevision,
...
#ifndef LUMINOPHORE_EFFECTS
    return actionError("LUMINOPHORE Shell projection is unavailable in this build", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
#else
    if (!Luminophore::shellProjection()->transact(surfaceNamespace, generation, revision, contentRevision, phase, requestedPlane, role, style))
        return actionError("LUMINOPHORE Shell projection transaction rejected", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
    return {};
#endif
}
...
static PHLWORKSPACE resolveWorkspaceForChange(const std::string& args) {
    ...
    const auto parsed  = getWorkspaceIDNameFromString(args);
    if (!monitor || parsed.id == WORKSPACE_INVALID)
        return nullptr;
    if (const auto existing = State::workspaceState()->query().id(parsed.id).run())
        return existing;
    return State::workspaceState()->create(parsed.id, monitor->m_id, parsed.name);
}
...
ActionResult Actions::spatialMoveView(Math::eDirection direction) {
    const auto converted = spatialDirection(direction);
    if (!converted)
        return actionError("Invalid LUMINOPHORE spatial direction", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::MOVE_VIEW, *converted) ?
        ActionResult{} :
        actionError("LUMINOPHORE View cannot move farther", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}

ActionResult Actions::spatialAdjustView(Math::eDirection direction, std::optional<PHLWINDOW> window) {
    const auto converted = spatialDirection(direction);
    if (!converted)
        return actionError("Invalid LUMINOPHORE spatial direction", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::ADJUST_VIEW, *converted, window) ?
        ActionResult{} :
        actionError("LUMINOPHORE View extent cannot change", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}

ActionResult Actions::spatialMoveWindow(Math::eDirection direction, std::optional<PHLWINDOW> window) {
    const auto converted = spatialDirection(direction);
    if (!converted)
        return actionError("Invalid LUMINOPHORE spatial direction", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::MOVE_WINDOW, *converted, window) ?
        ActionResult{} :
        actionError("LUMINOPHORE window cannot move farther", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}

ActionResult Actions::spatialFocusDirection(Math::eDirection direction, std::optional<PHLWINDOW> window) {
    const auto converted = spatialDirection(direction);
    if (!converted)
        return actionError("Invalid LUMINOPHORE spatial direction", eActionErrorLevel::WARNING, eActionErrorCode::INVALID_ARGUMENT);
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::FOCUS_DIRECTION, *converted, window) ?
        ActionResult{} :
        actionError("LUMINOPHORE directional focus cannot change", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}

ActionResult Actions::spatialToggleDesktop() {
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::TOGGLE_DESKTOP, eLuminophoreSpatialDirection::UP) ?
        ActionResult{} :
        actionError("LUMINOPHORE desktop presentation cannot change", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}

ActionResult Actions::spatialToggleWide(std::optional<PHLWINDOW> window) {
    return Luminophore::spatialRuntime()->dispatch(Luminophore::eSpatialAction::TOGGLE_WIDE, eLuminophoreSpatialDirection::UP, window) ?
        ActionResult{} :
        actionError("LUMINOPHORE WIDE mode cannot change", eActionErrorLevel::INFO, eActionErrorCode::INVALID_STATE);
}
