...
void CMonitorRuleManager::clear() {
    Luminophore::MonitorLayout::configReload();
    m_rules.clear();
}
...
CMonitorRule CMonitorRuleManager::get(const PHLMONITOR PMONITOR) {
    ...
    for (auto const& r : m_rules | std::views::reverse) {
        if (PMONITOR->matchesStaticSelector(r.m_name)) {
            auto resolved     = applyWlrOutputConfig(r);
            resolved.m_offset = Luminophore::MonitorLayout::configuredPosition(PMONITOR, resolved.m_offset);
            return resolved;
        }
    }
    ...
    fallbackRule.m_offset     = Vector2D{-INT32_MAX, -INT32_MAX};
    fallbackRule.m_scale      = -1;
    auto resolved             = applyWlrOutputConfig(fallbackRule);
    resolved.m_offset         = Luminophore::MonitorLayout::configuredPosition(PMONITOR, resolved.m_offset);
    return resolved;
}
