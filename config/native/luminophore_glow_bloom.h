...
/* Level zero is intentionally absent: the visible result begins at the first
 * downsampled level, so the emissive outline cannot become a crisp light ridge. */
static const char *luminophore_bloom_composite_source =
    "#extension GL_OES_standard_derivatives : enable\n"
    "precision highp float;varying vec2 v_uv;uniform sampler2D u_bloom;"
    "uniform vec4 u_tile;uniform vec4 u_panel;uniform float u_radius;"
    "uniform vec3 u_color;uniform float u_time;uniform float u_age;uniform float u_phase;uniform float u_intensity;uniform float u_reveal;"
    "float rounded_rect(vec2 p,vec2 h,float r){vec2 q=abs(p)-h+vec2(r);"
    "return min(max(q.x,q.y),0.0)+length(max(q,0.0))-r;}"
    "void main(){vec2 uv=(gl_FragCoord.xy-u_tile.xy)/u_tile.zw;"
    "if(uv.x<0.0||uv.y<0.0||uv.x>1.0||uv.y>1.0)discard;"
    "vec2 panel=max(vec2(1.0),u_panel.zw);"
    "vec2 p=uv*u_tile.zw-u_panel.xy;float d=rounded_rect(p,panel*0.5,min(u_radius,min(panel.x,panel.y)*0.5));"
    "float outside=smoothstep(-12.0,4.0,d);float character=fract(u_phase*4.17+0.31);"
    "float period=mix(4.9,6.4,fract(u_phase*2.73+0.17));float cycle=fract(u_time/period+u_phase);"
    "float rise=mix(0.32,0.44,character);float breath=smoothstep(0.0,rise,cycle)*(1.0-smoothstep(rise,1.0,cycle));"
    "float gain=mix(0.34,1.38,breath)*u_intensity*smoothstep(0.0,0.20,u_age);"
    "float a=(1.0-exp(-texture2D(u_bloom,uv).a*gain))*outside*u_reveal;"
...
static inline bool luminophore_bloom_init(struct luminophore_bloom_pyramid *bloom) {
    ...
    bloom->composite_program = luminophore_bloom_link(luminophore_bloom_composite_source, bloom->error, sizeof(bloom->error));
    bool shaders = bloom->emission_program && bloom->downsample_program &&
                   bloom->upsample_program && bloom->composite_program;
...
static inline void luminophore_bloom_destroy(struct luminophore_bloom_pyramid *bloom) {
    ...
    if (bloom->composite_program) glDeleteProgram(bloom->composite_program);
    memset(bloom, 0, sizeof(*bloom));
}
...
static inline bool luminophore_bloom_prepare(
    struct luminophore_bloom_pyramid *bloom, int panel_width, int panel_height,
    float corner_radius, float outline_width, float extent
) {
    if (!bloom || !bloom->available) return false;
    int padding = luminophore_bloom_required_padding(extent);
    int width = panel_width + padding * 2;
    int height = panel_height + padding * 2;
    bool exact_match = bloom->allocated && width == bloom->widths[0] && height == bloom->heights[0] &&
        padding == bloom->padding && panel_width == bloom->panel_width &&
        panel_height == bloom->panel_height &&
        fabsf(corner_radius - bloom->corner_radius) < 0.01f &&
        fabsf(outline_width - bloom->outline_width) < 0.01f &&
        fabsf(extent - bloom->extent) < 0.01f;
    ...
    if (bloom->allocated)
        luminophore_bloom_release_textures(bloom);
    return luminophore_bloom_prepare_capacity(
        bloom, panel_width, panel_height, 0.0f, 0.0f,
        panel_width, panel_height, corner_radius, outline_width, extent
    );
}
...
static inline bool luminophore_bloom_composite(
    struct luminophore_bloom_pyramid *bloom, GLuint buffer, int viewport_width, int viewport_height,
    float tile_x, float tile_y, float time, float age, float phase, float intensity, float reveal,
    float red, float green, float blue
) {
    if (!bloom || !bloom->allocated || bloom->dirty) return false;
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glViewport(0, 0, viewport_width, viewport_height);
    glEnable(GL_BLEND); glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
    glUseProgram(bloom->composite_program);
    glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D, bloom->textures[LUMINOPHORE_BLOOM_HALF]);
    glUniform1i(glGetUniformLocation(bloom->composite_program, "u_bloom"), 0);
    glUniform4f(glGetUniformLocation(bloom->composite_program, "u_tile"), tile_x, tile_y,
                (float)bloom->widths[0], (float)bloom->heights[0]);
    glUniform4f(glGetUniformLocation(bloom->composite_program, "u_panel"),
                bloom->panel_x, bloom->panel_y,
                (float)bloom->panel_width, (float)bloom->panel_height);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_radius"), bloom->corner_radius);
    glUniform3f(glGetUniformLocation(bloom->composite_program, "u_color"), red, green, blue);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_time"), time);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_age"), age);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_phase"), phase);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_intensity"), intensity);
    glUniform1f(glGetUniformLocation(bloom->composite_program, "u_reveal"), reveal);
    luminophore_bloom_quad(bloom->composite_program, buffer);
    bloom->rendered = glGetError() == GL_NO_ERROR;
    return bloom->rendered;
}
