#!/usr/bin/bash
# Real desktop package, disposable QEMU root only. Dependency resolution and
# graphical acceptance are separate from this transaction/relocation test.
set -euo pipefail
[[ $(< /proc/cmdline) == *luminophore_test=1* ]] || exit 90
finish() { echo "DESKTOP_PACKAGE_VM_RESULT=$?"; /usr/bin/sync; /usr/bin/busybox poweroff -f; }
trap finish EXIT
...
/usr/bin/python3 -I /fixtures/check-desktop.py
...
/usr/bin/python3 -I /fixtures/check-desktop.py
pm -R luminophore-compositor
echo DESKTOP_PACKAGE_MATRIX_PASS
